...
RunDMRcate <- function(betaVals_mat,
  ...
  dmrcate_out <- tryCatch(
    ...
    dmrcate(myannotation, lambda = lambda_int, C = C_int),
  ...
  if(!is.null(dmrcate_out)){
    ...
    gr <- extractRanges(dmrcate_out, genome = "hg19")
    dmrcateOut_df <- as.data.frame(gr)
...
WriteDMRcateResults <- function(beta_mat,
  ...
  resultsDir <- "Users/Asus/Documents/GSEs/Simulation/DMRCate/"
  ...
  for(i in 1:nrow(designPts_mat)){
    ...
    for(j in 1:nrow(paramsGrid_mat)){
              ...
              file_char <- paste0(resultsDir,
                                  "DMRcateResults_delta", delta,
                                  "_seed", seed,
                                  "_lambda", lambda, 
                                  "_C", C_int, ".RDS")
...
#### Setting the results dir
resultsDir <- "/path/to/your/file/Simulation/DMRCate/"
...
ProcessDMRcateResults <- function(resultsDir,
  ...
  files_char <- list.files(path = resultsDir, pattern = "^DMRcateResults_.*\\.RDS$")
  ...
  for(i in 1:nrow(designPts_mat)){
    ...
    for(j in 1:nrow(paramsGrid_mat)){
      ...
      ###  Load Results  ###
      resFileName_char <- paste0(resultsDir,
                                  "DMRcateResults_delta", delta,
                                  "_seed", seed,
                                  "_lambda", lambda, 
                                  "_C", C_int, ".RDS")
...
out_df <- ProcessDMRcateResults(resultsDir = "path/to/Simulation/DMRcate/",
                                beta_mat,
                                Aclusters_df,
                                verbose = TRUE)


write.csv(out_df, file = paste0("/path/to/Simulation/DMRcate/","clean_DMR_cate_Simulation.csv"))
