...
####### function needed, but not modified except Standardize output for idDMR ############################################
...
################## Function to Run Bumphunter ###############
RunBumphunter <- function(betaVals_mat,
                          labels_fct = factor(c(rep("Treatment", 21),
                                                rep("Control", 20))),### the treatement are ordered in the preprocessing step
                          chromos_char,
                          chromPosit_num,
                          cpgLocation_df,
                          pickCutoffQ_num,
...
resultsDir <- "/path/to/Simulation/Bumphunter/"
WriteBumphunterResults <- function(beta_mat,
                                   ...
                                   cutoffQ_num = c( 0.95, 0.99),
                                   maxGap_int = c(200, 250, 500, 750, 1000),
                                   resultsDir = "Bumphunter/",
                                   verbose = TRUE){
...
resultsDir <- "/path/to/your/files/Simulation/Bumphunter/"
...
WriteBumphunterResults(beta_mat,
                       ...
                       Aclusters_df,
                       deltas_num = c(0, 0.3),
                       seeds_int = c(100, 210, 330),
                       cutoffQ_num = c(0.95, 0.99),
                       maxGap_int = c(500, 750, 1000),
...
Bumphunter_simu <- ProcessBumphunterResults(resultsDir,
                                    beta_mat,
                                    Aclusters_df,
                                    verbose = TRUE)

write.csv(Bumphunter_simu, file = paste0(resultsDir,"clean_DMR_Bumphunter_Simulation.csv"))
