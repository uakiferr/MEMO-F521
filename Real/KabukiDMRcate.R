...
Kabuki_dmr_df <- read.csv(paste0(data.dir, "Kabuki_DMR_to_find.csv"),
                          header = TRUE)
...
MergeDMRsWithCPGs <- function(DMRs_df, CPGs_df, alpha = 0.05){
  ...
  if (nrow(DMRs_df) == 0) {
    message("No DMRs found with p-value less than alpha")
    merged_df <- NULL 
  } else {
  ...
  if (nrow(overlaps_df) == 0) {
    message("No overlaps found between DMRs and CpGs")
    merged_df <- NULL
  } else {
  ...
  merged_df <- merge(overlapInfo_df, numCPGs,
...
RunDMRcate <- function(betaVals_mat,
 ...
 # Parallel no supported on Windows
  ...
  if(!is.null(dmrcate_out)){
    ...
    gr <- tryCatch(extractRanges(dmrcate_out, genome = "hg19"), 
  ...
  for(i in 1:nrow(designPts_mat)){
    ...
    for(j in 1:nrow(paramsGrid_mat)){
      ...
      file_char <- paste0(resultsDir,
                          "DMRcateResults_samplesize", size,
                          "_seed", seed,
                          "_lambda", lambda, 
                          "_C", C_int, ".RDS")
