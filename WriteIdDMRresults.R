...
rownames(Aclusters_df) <- Aclusters_df$cpg # making sure the row are the cgnames 
...
resultsDir <- "/path/to/your/files/Simulation/idDMR/"
...
### Fonction to extract ranges from iDDMR, is different from DMRcate
### so I changed only the name of this function. 
...
idDMR_simu <- ProcessidDMRResults(resultsDir= resultsDir,
                                       beta_mat,
                                       Aclusters_df,
                                       verbose = TRUE)

write.csv(idDMR_simu , file = paste0(resultsDir,"clean_DMR_idDMR_Simulation.csv"))
